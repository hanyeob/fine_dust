...
def add_time(df):
    ...
    temp = (df.time - (12 * 60 + 30)).abs()
    df["lunch_timediff"] = temp / temp.max() * 1
    return df
...
def scaling(df):
    ...
    scaled_data = pd.concat([scaled_data, df['lunch_timediff']], axis=1)
    return scaled_data
...
def main():
    ...
    for i in classlist:
        ...
        scaled_data.to_csv(f"./scaled_data/{i}_MA_timediff_scaled.csv")
